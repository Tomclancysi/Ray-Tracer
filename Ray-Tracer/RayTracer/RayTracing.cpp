...
namespace RayTracer
{
	...
	void RayTracing::initialize(int w, int h, int c)
	{
		m_config.m_width = w;
		m_config.m_height = h;
	...
	unsigned char *RayTracing::render()
	{
		// viewport
	...
	void RayTracing::drawPixel(unsigned int x, unsigned int y, const Vector4D &color)
	{
	...
	void RayTracing::setRecursionDepth(int depth)
	{
		m_config.m_maxDepth = depth;
	}

	void RayTracing::setCamera(const Vector3D & cameraPos, const Vector3D & target, const Vector3D & worldUp, float fovy, float aspect, float aperture, float focus_dist)
	{
	}

	Hitable *RayTracing::randomScene()
	{
		int n = 500;
	...
	Vector4D RayTracing::tracing(const Ray &r, Hitable *world, int depth)
	{
		HitRecord rec;
	...
	float RayTracing::hitSphere(const Vector3D &center, const float &radius, const Ray &ray)
	{
